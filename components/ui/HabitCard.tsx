...
export const HabitCard = ({ habit }: { habit: Habit }) => {
  ...
  const iconName = (habit.icon || "leaf") as keyof typeof Ionicons.glyphMap;
    ...
    <View className="bg-[#1C1C1E] rounded-3xl p-4 mb-4 border border-gray-800">
      ...
      <View className="flex-row justify-between items-start mb-4">
        <View className="flex-1">
          <Text className="text-white font-bold text-lg mb-1">{habit.title}</Text>
          <Text className="text-gray-500 text-xs">{getFrequencyText()}</Text>
        </View>
        <View className="bg-gray-800 p-2 rounded-xl ml-2">
            <Ionicons name={iconName} size={18} color={habit.color || "#22C55E"} />
        </View>
      </View>
      ...
      <View className="flex-row justify-between mb-4">
        {weekDays.map((date, index) => {
          ...
          return <DayItem key={index} date={date} status={status as HabitStatus} />;
        })}
      </View>
      ...
      <View className="flex-row justify-between items-center border-t border-gray-800 pt-3">
        ...
        <View className="flex-row items-center space-x-4">
          <View className="flex-row items-center">
            ...
            <Text className="text-gray-300 text-xs ml-1 font-bold">{habit.currentStreak || 0}</Text>
          </View>
          <View className="flex-row items-center ml-3">
            <Ionicons name="checkmark-circle-outline" size={16} color="#F59E0B" />
            <Text className="text-gray-300 text-xs ml-1 font-bold">{completionRate}%</Text>
          </View>
        </View>
